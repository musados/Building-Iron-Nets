...
ברירת המחדל היא `claude-opus-4-8`. אפשר לעבור ל-Claude Fable 5 (המודל החזק
ביותר) דרך `.env`:
...
הערות ל-Fable: דורש ארגון עם שמירת נתונים של 30 יום; אם מסווגי הבטיחות שלו
מסרבים לבקשה, השרת מפעיל fallback צד-שרת אוטומטי ל-Opus 4.8 באותה קריאה.

## עלויות

כל חילוץ שולח את ה-PDF ל-Claude API. העלות תלויה בגודל התוכנית — בדרך כלל
סנטים בודדים עד ~$1 לתוכנית מרובת עמודים ב-Opus (‏$5/$25 למיליון טוקנים);
‏Fable 5 יקר פי שניים (‏$10/$50).
